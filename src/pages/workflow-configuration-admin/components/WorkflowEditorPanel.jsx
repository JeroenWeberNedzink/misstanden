...
function CollapsibleStepHeader({
  number,
  label,
  title,
  description,
      ...
      >
        ...
        <span className="min-w-0 block">
          <span className="flex items-center gap-2 mb-1.5">
            <span className="text-base font-bold text-sky-700">{label}</span>
            <Icon name={expanded ? 'ChevronUp' : 'ChevronDown'} size={16} className="text-sky-700 shrink-0" />
          </span>
          <span className="text-base font-semibold text-foreground block truncate">{title}</span>
          {description && (
            <span className="text-sm text-muted-foreground mt-1 block line-clamp-2">{description}</span>
          )}
        </span>
      </button>
    ...
    <div className="bg-transparent">
      <div className="pb-3 border-b border-border">
        <CollapsibleStepHeader
          number={2}
          label="Stap 2 - Bewerk workflow"
          title={workflow?.name || 'Workflow'}
          description={workflow?.description || 'Werk de naam en omschrijving bij voor dagelijkse administratie.'}
          expanded={expandedSteps.basics}
      ...
      <div className="pt-3 space-y-5">
        ...
        <section className="pt-4 border-t border-sky-100">
          <CollapsibleStepHeader
            number={3}
            label="Stap 3 - Workflow stappen"
            title="Statussen en transities"
            description={expandedSteps.statuses ? 'Beheer de route van een ticket door het proces.' : null}
            expanded={expandedSteps.statuses}
            onToggle={() => toggleStep('statuses')}
            meta={Number.isFinite(statusCount) ? (
          ...
          />
          {expandedSteps.statuses && (
            <p className="mt-3 text-xs text-muted-foreground">
              Open de stappeneditor om statussen, volgorde, doorstroom en SLA-duur te beheren.
            </p>
          )}
        </section>
        ...
        <section className="pt-4 border-t border-sky-100">
          <CollapsibleStepHeader
            ...
            label="Stap 4 - Team"
            title="Handlers"
            description={expandedSteps.team ? 'Koppel de behandelaars die voor deze workflow tickets mogen verwerken.' : null}
            expanded={expandedSteps.team}
            onToggle={() => toggleStep('team')}
            actions={(
            <Button
              variant="outline"
              size="sm"
          ...
          {expandedSteps.team && (
            <p className="mt-3 text-xs text-muted-foreground">
              Beheer welke behandelaars deze workflow mogen zien, oppakken of opvolgen.
            </p>
          )}
        </section>
