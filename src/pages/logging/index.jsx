...
const buildDiffRows = (oldData = {}, newData = {}) => {
  ...
  // nice ordering: changed first, then added, then removed
...
// ------------------------------
// Inline Audit Logs Table + Drawer
// ------------------------------
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            {logs.map((l) => {
              ...
              return (
                <tr key={l.event_id} className="hover:bg-muted/30 transition-smooth">
...
function AuditDetailsDrawer({ open, log, onClose }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-[10001] w-full max-w-3xl bg-card border-l border-border shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2 rounded-xl bg-background border border-border text-sm outline-none focus:ring-2 focus:ring-primary/30"
                />
                {query && (
                  <button
                    onClick={() => setQuery('')}
            ...
            <div className="border border-border rounded-xl overflow-hidden">
              ...
              {filteredRows.length === 0 ? (
                ...
                <div className="divide-y divide-border">
                  {filteredRows.map((r) => (
                    <div key={r.key} className="p-3 hover:bg-muted/20 transition-smooth">
            ...
            {log.meta && (
              <div className="border border-border rounded-xl overflow-hidden">
                <button
                  ...
                  className="w-full px-3 py-2 bg-muted/30 border-b border-border flex items-center justify-between"
                >
                  <div className="flex items-center gap-2">
...
export default function AuditLogsAdminPage() {
  const defaultFilters = useMemo(
    () => ({
      schemaName: 'all',        // optional if you want it
      tableName: 'all',
      operation: 'all',
      search: '',
  ...
  const tableOptions = useMemo(() => {
    // Optional: hardcode the tables you care about (clean UX)
    const base = [
  ...
  const loadAudit = async ({ showSpinner = true } = {}) => {
    ...
    try {
      ...
      setError(e?.message || 'Fout bij laden audit logs');
      setLogs([]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <>
      ...
      <AuthContextNavigator>
        <div className="min-h-screen app-page-gradient bg-background py-6 px-4 sm:px-6 lg:px-8 pt-10">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Error */}
            {error && (
              <div className="card bg-error/10 border-error mb-6 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <Button variant="ghost" size="sm" iconName="X" iconPosition="left" onClick={() => setError('')}>
                    Sluiten
                  </Button>
                </div>
              </div>
