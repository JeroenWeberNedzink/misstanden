      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 mt-5">
        <div className="mx-auto w-full max-w-4xl px-4 pt-20 pb-32 sm:pb-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 sm:mb-8">
            ...
            {/* Report Summary */}
            <div className="rounded-2xl bg-card border border-border p-6 shadow-sm hover:shadow-md transition-shadow animate-in fade-in slide-in-from-left-4 duration-500 delay-400">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                  <Icon name="FileText" size={20} className="text-primary" />
                </div>
                <h3 className="text-lg font-bold text-foreground">
                  {t('reportConfirmation.reportSummary.title')}
                </h3>
              </div>

              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                  <Icon name="Workflow" size={18} className="text-muted-foreground" />
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] uppercase tracking-wide font-semibold text-muted-foreground mb-0.5">
                      {t('reportConfirmation.reportSummary.type')}
                    </p>
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] uppercase tracking-wide font-semibold text-muted-foreground mb-0.5">
                      {t('reportConfirmation.reportSummary.severity')}
                    </p>
                    <p className="text-sm font-medium text-foreground">{severityLabel}</p>
                  </div>
                </div>
                ...
                {locationLabel ? (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] uppercase tracking-wide font-semibold text-muted-foreground mb-0.5">
                        {t('reportConfirmation.reportSummary.location')}
                      </p>
                ...
                <div className="pt-2">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] uppercase tracking-wide font-semibold text-muted-foreground">
                      {t('reportConfirmation.reportSummary.description')}
                    </p>
                    {ticketInfo?.description?.length > 120 ? (
                      <button
                        type="button"
                        onClick={() => setShowFullDesc((v) => !v)}
                        className="text-xs text-primary hover:underline font-medium inline-flex items-center gap-1"
                      >
                        {showFullDesc ? 'Minder' : 'Meer'}
                        <Icon name={showFullDesc ? 'ChevronUp' : 'ChevronDown'} size={12} />
                      </button>
                    ) : null}
                  </div>
            ...
            {/* Next Steps */}
            <div className="rounded-2xl bg-card border border-border p-6 shadow-sm hover:shadow-md transition-shadow animate-in fade-in slide-in-from-right-4 duration-500 delay-400">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                  <Icon name="ListChecks" size={20} className="text-primary" />
                </div>
                <h3 className="text-lg font-bold text-foreground">
                  {t('reportConfirmation.nextSteps.title')}
                </h3>
              </div>
              ...
              <div className="space-y-3">
                {steps.map((s, idx) => (
                  <div
                    key={s.n}
                    className="flex gap-3 p-4 rounded-xl border border-border bg-muted/10 hover:bg-muted/20 transition-colors"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start gap-2">
                        <Icon name={s.icon} size={16} className="text-primary mt-0.5" />
